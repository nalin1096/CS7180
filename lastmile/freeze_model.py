...
def freeze_sony_model(model):
    ...
    layer1a = model.get_layer(index=1)
    layer1a.trainable = False
    x1a = layer1a(x0)

    layer1b = model.get_layer(index=2)
    layer1b.trainable = False
    x1b = layer1b(x1a)

    layer1c = model.get_layer(index=3)
    layer1c.trainable = False
    x1c = layer1c(x1b)

    ## Block 2

    layer2a = model.get_layer(index=4)
    layer2a.trainable = False
    x2a = layer2a(x1c)

    layer2b = model.get_layer(index=5)
    layer2b.trainable = False
    x2b = layer2b(x2a)

    layer2c = model.get_layer(index=6)
    layer2c.trainable = False
    x2c = layer2c(x2b)

    ## Block 3

    layer3a = model.get_layer(index=7)
    layer3a.trainable = False
    x3a = layer3a(x2c)

    layer3b = model.get_layer(index=8)
    layer3b.trainable = False
    x3b = layer3b(x3a)

    layer3c = model.get_layer(index=9)
    layer3c.trainable = False
    x3c = layer3c(x3b)

    ## Block 4

    layer4a = model.get_layer(index=10)
    layer4a.trainable = False
    x4a = layer4a(x3c)

    layer4b = model.get_layer(index=11)
    layer4b.trainable = False
    x4b = layer4b(x4a)

    layer4c = model.get_layer(index=12)
    layer4c.trainable = False
    x4c = layer4c(x4b)

    ## Block 5

    layer5a = model.get_layer(index=13)
    layer5a.trainable = False
    x5a = layer5a(x4c)

    layer5b = model.get_layer(index=14)
    layer5b.trainable = False
    x5b = layer5b(x5a)

    ## Block 6

    layer6a = model.get_layer(index=15)
    layer6a.trainable = False
    x6a = layer6a(x5b)

    layer6b = model.get_layer(index=16)
    layer6b.trainable = False
    x6b = layer6b(x6a)

    layer6c = model.get_layer(index=17)
    layer6c.trainable = False
    x6c = layer6c(x6b)

    ## Block 7

    layer7a = model.get_layer(index=18)
    layer7a.trainable = False
    x7a = layer7a(x6c)

    layer7b = model.get_layer(index=19)
    layer7b.trainable = False
    x7b = layer7b(x7a)

    layer7c = model.get_layer(index=20)
    layer7c.trainable = False
    x7c = layer7c(x7b)

    ## Block 8

    layer8a = model.get_layer(index=21)
    layer8a.trainable = False
    x8a = layer8a(x7c)

    layer8b = model.get_layer(index=22)
    layer8b.trainable = False
    x8b = layer8b(x8a)

    layer8c = model.get_layer(index=23)
    layer8c.trainable = False
    x8c = layer8c(x8b)

    ## Block 9

    layer9a = model.get_layer(index=24)
    layer9a.trainable = True
    x9a = layer9a(x8c)

    layer9b = model.get_layer(index=25)
    layer9b.trainable = True
    x9b = layer9b(x9a)

    layer9c = model.get_layer(index=26)
    layer9c.trainable = True
    x9c = layer9c(x9b)

    ## Block 10

    layer10a = model.get_layer(index=27)
